...
public class SpinMe : MonoBehaviour {
	...
	void Update () {
		...
		/*Another option
		 * float xDegreesPerSecond = (Time.deltaTime * xRotationsPerMinute) * 6;
		transform.RotateAround ( transform.position, transform.right, xDegreesPerSecond );

		float yDegreesPerFrame = (Time.deltaTime * yRotationsPerMinute) * 6;
		transform.RotateAround ( transform.position, transform.up, yDegreesPerFrame );

		float zDegreesPerSecond = (Time.deltaTime * zRotationsPerMinute) * 6; ;
		transform.RotateAround ( transform.position, transform.forward, zDegreesPerSecond );*/
	}
}
